...
static Instruction *SimplifyNVVMIntrinsic(IntrinsicInst *II, InstCombiner &IC) {
  ...
  const SimplifyAction Action = [II]() -> SimplifyAction {
    ...
    case Intrinsic::nvvm_sqrt_rn_f:
      return {SPC_Sqrt, FTZ_MustBeOff};
    case Intrinsic::nvvm_sqrt_rn_ftz_f:
      return {SPC_Sqrt, FTZ_MustBeOn};
  ...
  case SPC_Sqrt: {
    ...
    Value *Arg = II->getArgOperand(0);
    Type *Ty = Arg->getType();
    Value *Cmp = IC.Builder->CreateFCmpOLT(Arg, ConstantFP::get(Ty, "-0.0"));
    Value *Sqrt = IC.Builder->CreateCall(
        Intrinsic::getDeclaration(M, Intrinsic::sqrt, {Ty}), {Arg}, "sqrt");
    return SelectInst::Create(Cmp, ConstantFP::getNaN(Ty), Sqrt, "sqrt.sel");
  }
  }
}
