...
; CHECK: [[sqrt_rn_d_cmp:%[a-zA-Z0-9.]+]] = fcmp olt double %a, -0.0
; CHECK: [[sqrt_rn_d:%[a-zA-Z0-9.]+]] = call double @llvm.sqrt.f64(double %a)
; CHECK: select i1 [[sqrt_rn_d_cmp]], double 0x7FF8000000000000, double [[sqrt_rn_d]]
  %ret = call double @llvm.nvvm.sqrt.rn.d(double %a)
  ret double %ret
}
; nvvm.sqrt.f is a special case: It goes to a llvm.sqrt.f + select idiom
; regardless of whether ftz is enabled.
; CHECK-LABEL: @test_sqrt_f
define float @test_sqrt_f(float %a) #0 {
; CHECK: [[sqrt_f_cmp:%[a-zA-Z0-9.]+]] = fcmp olt float %a, -0.0
; CHECK: [[sqrt_f:%[a-zA-Z0-9.]+]] = call float @llvm.sqrt.f32(float %a)
; CHECK: select i1 [[sqrt_f_cmp]], float 0x7FF8000000000000, float [[sqrt_f]]
  %ret = call float @llvm.nvvm.sqrt.f(float %a)
  ret float %ret
}
; CHECK-LABEL: @test_sqrt_rn_f
define float @test_sqrt_rn_f(float %a) #0 {
; NOFTZ: [[sqrt_rn_f_cmp:%[a-zA-Z0-9.]+]] = fcmp olt float %a, -0.0
; NOFTZ: [[sqrt_rn_f:%[a-zA-Z0-9.]+]] = call float @llvm.sqrt.f32(float %a)
; NOFTZ: select i1 [[sqrt_rn_f_cmp]], float 0x7FF8000000000000, float [[sqrt_rn_f]]
...
; FTZ: [[sqrt_rn_ftz_f_cmp:%[a-zA-Z0-9.]+]] = fcmp olt float %a, -0.0
; FTZ: [[sqrt_rn_ftz_f:%[a-zA-Z0-9.]+]] = call float @llvm.sqrt.f32(float %a)
; FTZ: select i1 [[sqrt_rn_ftz_f_cmp]], float 0x7FF8000000000000, float [[sqrt_rn_ftz_f]]
