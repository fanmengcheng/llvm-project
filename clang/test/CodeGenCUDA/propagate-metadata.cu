...
// RUN: %clang_cc1 -x c++ -emit-llvm-bc -DLIB %s -o %t.bc -triple nvptx-unknown-unknown
...
// RUN:   -fcuda-flush-denormals-to-zero -o - -fcuda-is-device \
// RUN:   -triple nvptx-unknown-unknown \
// RUN: | FileCheck %s --check-prefix=CHECK --check-prefix=FTZ \
// RUN:   --check-prefix=NOFAST

// RUN: %clang_cc1 -x cuda %s -emit-llvm -mlink-cuda-bitcode %t.bc \
// RUN:   -fcuda-flush-denormals-to-zero -o - -fcuda-is-device \
// RUN:   -menable-unsafe-fp-math -triple nvptx-unknown-unknown \
